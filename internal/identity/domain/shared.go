// Package domain holds the identity context's domain model: entities, value
// objects, domain errors, and — when logic spans more than one entity — the
// cross-entity domain services that live at the domain root.
package domain
...
// the domain events it has recorded. ID stays strongly typed (account.ID)
// through the type parameter, so embedding does not weaken identity.
