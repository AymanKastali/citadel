...
// ExpiresAt is the instant after which the token is no longer usable, as a value
// object: immutable and self-validating. It wraps a stdlib time.Time (stdlib time
// is allowed in the domain; third-party packages are not). The domain does no I/O
// and reads no clock — "now" is passed into the commands/queries that need it,
// never fetched here.
...
// time; it applies no other bound — whether the instant is in the past is a
// runtime question answered by the entity's IsExpired(now), not a construction
// invariant (a token may legitimately be reconstituted after it has expired).
